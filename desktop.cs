//================================================
// Title: Blocktop (Glass Desktop)
//================================================
exec("config/Blocktop/prefs.cs");
new AudioProfile(BLG_Sound_Click)
{
	fileName = "Add-Ons/System_Blocktop/sound/click.wav";
...
function BLG_DT::setEnabled(%this, %bool) {
	...
	if(!%bool) {
		messageBoxOk("Restart", "Sorry that you don't want it. You need to restart Blockland now");
	}
}
...
function BLG_DT::populateBackgroundList(%this) {
	...
	%file = findFirstFile("config/Blocktop/background/*.jpg");
	while(%file !$= "") {
	    %bitmap = new GuiBitmapCtrl() {
			profile = "GuiDefaultProfile";
			horizSizing = "right";
		...
		%i++;
		%file = findNextFile("config/Blocktop/background/*.jpg");
	}
...
function BLG_DT::proccessSettings(%this) {
	...
	%bg.bitmap = $BLGD::Background $= "" ? "Add-Ons/System_Blocktop/image/desktop.jpg" : $BLGD::Background;
	%g = %bg.getGroup();
	%g.remove(%bg);
	%g.add(%bg);
}
...
function BLG_DT::toggleMenu(%this) {
	...
	if(!%menu.isAnimating) {
		if(%menu.open) {
			...
			BLG_DT_CAS.newAnimation(%menu).setDuration(100).setPosition(5 SPC getWord(Canvas.getExtent(), 1)-59).setExtent("54 54").setColor("0 0 0 127").setFinishHandle("BLG_Desktop_Menu.onAnimationComplete").start();
		} else {
			%menu.isAnimating = true;
			BLG_DT_CAS.newAnimation(%menu).setDuration(100).setPosition((getWord(Canvas.getExtent(), 0)/2)-320 SPC (getWord(Canvas.getExtent(), 1)/2)-240).setExtent("640 480").setColor("255 255 255 255").setFinishHandle("BLG_Desktop_Menu.onAnimationComplete").start();
		}
		%menu.open = !%menu.open;
	}
}
...
function BLG_DT::loadAppData(%this) {
	%fo = new FileObject();
	%fo.openForRead("config/Blocktop/save.dat");
	while(!%fo.isEOF()) {
		%line = %fo.readLine();
	...
	%file = findFirstFile("config/Bloctop/apps/*/main.cs");
	while(%file !$= "") {
		if(strCount(%file, "/") == 4) {
			exec(%file);
		} else {
		...
		%file = findNextFile("config/Bloctop/apps/*/main.cs");
	}
}

function BLG_DT::saveAppData(%this) {
	%fo = new FileObject();
	%fo.openForWrite("config/Blocktop/save.dat");
...
function BLG_DT::newAppIcon(%this, %name, %eval, %x, %y, %mode) {
    ...
    if(%icon.type $= "basic" && %icon.dat !$= "") {
		BLG_DT_CAS.newAnimation(%gui).setDuration(1000).setPosition(5 + 64 * %x SPC 5 + 64 * %y).setExtent("54 54").setColor(%icon.dat).start();
    } else {  	
		BLG_DT_CAS.newAnimation(%gui).setDuration(1000).setPosition(5 + 64 * %x SPC 5 + 64 * %y).setExtent("54 54").start();
    }
...
function BLG_DT::startScreenSaver(%this) {
	...
	for(%i = 0; %i < BLG_DT_IconGroup.getCount(); %i++) {
		...
		if(%i == 0) {
			BLG_DT_CAS.newAnimation(%obj.gui).setDuration(750).setPosition(getWord(%center, 0)+getWord(%p, 0)-15 SPC getWord(%center, 1)+getWord(%p, 1)-15).setExtent(%iconSize).setFinishHandle("BLG_DT.screenSaverLoop").start();
		} else {
			BLG_DT_CAS.newAnimation(%obj.gui).setDuration(750).setPosition(getWord(%center, 0)+getWord(%p, 0)-15 SPC getWord(%center, 1)+getWord(%p, 1)-15).setExtent(%iconSize).start();
		}
	}
...
function BLG_DT::stopScreensaver(%this) {
	...
	for(%i = 0; %i < BLG_DT_IconGroup.getCount(); %i++) {
		%obj = BLG_DT_IconGroup.getObject(%i);
		BLG_DT_CAS.newAnimation(%obj.gui).setDuration(200).setPosition(5 + 64 * getWord(%obj.gridPos, 0) SPC 5 + 64 * getWord(%obj.gridPos, 1)).setExtent("54 54").start();
	}	
}
...
if(!isObject(BLG_DT_CAS)) {
	new ScriptGroup(BLG_DT_CAS) {
		isRunning = false;
	};
}

function BLG_DT_CAS::newAnimation(%this, %object) {
...
function BLG_DT_CAS::start(%this) {
	if(%this.isRunning) {
		return;
	}
...
function BLG_Animation::start(%this) {
	BLG_DT_CAS.add(%this);
	if(!BLG_DT_CAS.isRunning && BLG_DT_CAS.start $= "") {
		BLG_DT_CAS.start = BLG_DT_CAS.schedule(5, start);
	}
}
			...
			if(isObject(%obj)) {
	            if(%obj.gui.position $= %obj.originalPos) {
	                ...
	                if(BLG_DT.appGrid[%gridX, %gridY] $= "") {
						...
						BLG_DT_CAS.newAnimation(%obj.gui).setDuration(100).setPosition(5 + 64 * %gridX SPC 5 + 64 * %gridY).start();
	                } else {
						...
						BLG_DT_CAS.newAnimation(%curObj.gui).setDuration(100).setPosition(5 + 64 * getWord(%curObj.gridPos, 0) SPC 5 + 64 * getWord(%curObj.gridPos, 1)).start();
						BLG_DT_CAS.newAnimation(%obj.gui).setDuration(100).setPosition(5 + 64 * getWord(%obj.gridPos, 0) SPC 5 + 64 * getWord(%obj.gridPos, 1)).start();
	                }
